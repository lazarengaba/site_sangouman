<?php
    require_once "../../requiredPages/connect.php";
        ...
        $check_req ="SELECT * FROM images_sous_menus";
    ...
    if (isset($_FILES['img_sous_menu'])&&$_FILES['img_sous_menu']!="") {
		...
		if ($_FILES['img_sous_menu']['size'] <= $taille_Max) {
			...
			if (in_array($extensions_Upload, $extensions_Valides)) {
				...
				$chemin = "../../photo_upload/".($check_val['id'] + 1).".".$extensions_Upload;
				$export = move_uploaded_file($_FILES['img_sous_menu']['tmp_name'], $chemin);

				if ($export) {
					$fichier = ($check_val['id'] + 1).".".$extensions_Upload;

					$photo = $bdd->prepare("INSERT INTO images_sous_menus (id_sous_menu, img, desc_img)
                                            VALUES (?,?,?)");
					$photo_exe=$photo->execute(array($_POST['idSousMenuModif'], $fichier, $_POST['desc_img']));
				}
			} else {
    ...
    $req=$bdd->prepare("UPDATE sous_menus SET text = ? WHERE id = ?");
    $req_exe=$req->execute(array($_POST['text'], $_POST['idSousMenuModif']));
    $req->closeCursor();    
